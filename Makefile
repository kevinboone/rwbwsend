...
# romwbw.zas is the only human-generated assembly-language file in the
#   program.
romwbw.obj: romwbw.zas
	cpm zas romwbw.zas


$(TARGET): $(OBJS) romwbw.obj
	@./rename_objs.sh $(OBJS)
	cpm linq -C100H -O$(TARGET) $(CRT) "_*.obj" romwbw.obj libc.lib

clean:
	rm -f *.pre *.p1 *.asm *.as2 _*.obj romwbw.obj $(OBJS) '$$'*
